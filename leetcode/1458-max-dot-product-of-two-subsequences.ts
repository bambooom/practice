...
function maxDotProduct(nums1: number[], nums2: number[]): number {
  ...
  // Initialize a 2D array to store the maximum dot product for each subarray
  const dp = new Array(n + 1)
    .fill(-Infinity)
    .map(() => Array(m + 1).fill(-Infinity));

  // helper function recursively calculates the maximum dot product for a given subarray
